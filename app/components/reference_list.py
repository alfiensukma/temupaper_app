...
class ReferenceListView(UnicornView):
    paper_id = ""
    search_query = ""
    papers = []
    ...
    def load_data(self):
        ...
        try:
            ...
            with driver.session() as session:
                result = session.run("""
                    MATCH (p:Paper {paperId: $paperId})-[:REFERENCES]->(reference:Paper)
                    OPTIONAL MATCH (reference)-[:AUTHORED_BY]->(author:Author)
                    RETURN 
                    reference.paperId AS paperId,
                    reference.title AS title,
                    collect(author.name) AS authors,
                    reference.year AS year,
                    reference.publicationDate AS date,
                    reference.abstract as abstract
